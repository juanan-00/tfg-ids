...
logger = logging.getLogger('logger_modelo_svm')


# Columnas a eliminar porque no se pueden monitorizar u obtener de pyflowmeter
...
class modeloSVM:
    """Suport vector machine para clasificacion multiclase"""
    
    def __init__(self, path_modelo=None):
        self.modelo = None
        self.scaler = None
        self.scaler_categorico = None
        self.col_features = []
        self.feature_selector = None
        self.features_seleccionadas= None
    ...
    def select_cat_scaler(self, tipo):
        """Selecciona tipo de scaler categorico"""
        if tipo == "label_encoder":
            scaler = LabelEncoder()
        elif tipo == "one_hot":
    ...
    def seleccionar_best_features(self, X, y):
        ...
        logger.info(f"Seleccionando mejores features con {self.feature_selector}")
        # KBest
        if self.feature_selector == "kbest":
            # info_mutual_classif | f_score
        ...
        self.features_seleccionadas = mejores_features
        return mejores_features

    def entrenar_modelo(self, X_train, X_test, y_train, y_test):
        """Se encarga de entrenar el modelo"""
        logger.info("Entrando modelo SVC")
        
        # parámetros adaptados para un menor tiempo computacional
        modelo_svm = SVC(
        ...
        #y_pred_proba = modelo_svm.predict_proba(X_test)  # Obtener probabilidades
        
        # Usar datos de prueba correctos
        self.obtener_resultados_clasificacion(y_test, y_pred) 
        
        # Calcular y visualizar ROC curve
        #fpr, tpr, roc_auc, clases = self.calcular_roc_auc(y_test, y_pred_proba)
        #self.visualizar_roc_curve(fpr, tpr, roc_auc, clases)

        # Se guarda el modelo en el path especificado
        joblib.dump(modelo_svm, "models/no_balanceado/modelo_svm_nobal.plk")

        return modelo_svm


    def cross_validation(self, X, y):
        logger.info("Aplicando cross_validation para obtener hiperparámetros")

        skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
        svm_temp = SVC(kernel='rbf', C=1.0, probability=True)

        metricas = ['accuracy', 'precision_weighted', 'recall_weighted', 'f1_weighted']
        resultados = {}

        for metrica in metricas:
            scores = cross_val_score(svm_temp, X, y, cv=skf, scoring=metrica, n_jobs=1)
            resultados[metrica] = {
                'media': scores.mean(),
                'std': scores.std(),
                'scores': scores
            }
            logger.info(f"CV {metrica}: {scores.mean():.4f} (+/- {scores.std()*2:.4f})")

        return hiperparametros

    def optimizar_hiperparametros(self, X_train, y_train):
        """Optimiza hiperparámtros del SVM utilizando GridSearchCV"""
        logger.info("Iniciando optimizacion de hiperparámetros con GridSearchCV")
        ...
        logger.info("Ejecutando busqueda de hiperparametros")
        grid_search.fit(X_train, y_train)
        logger.info(f"Mejores hiperparametros: {gird_search.best_params_}")
        logger.info(f"Mejor f1-score: {grid_search.best_score_:.4f}")
    ...
    def calcular_roc_auc(self, y_test, y_pred_proba):
        ...
        # Si solo hay 2 clases, label_binarize devuelve array 1D
        if n_clases == 2:
        ...
        print(f"\n{'='*60}")
        print("RESULTADOS ROC CURVE Y AUC SCORE")
        print(f"{'='*60}")
    ...
    def visualizar_roc_curve(self, fpr, tpr, roc_auc, clases):
        ...
        plt.title('ROC Curves - Support Vector Machine')
        plt.legend(loc="lower right")
        plt.grid(True, alpha=0.3)
        
        # Guardar gráfico
        #plt.savefig("../../modelo_svm_roc_curve.png", dpi=300, bbox_inches='tight')
        plt.show()
        
        print(f"\nGráfico ROC curve guardado en: modelo_svm_roc_curve.png")
    ...
    def obtener_resultados_clasificacion(self, y_test, y_pred, y_pred_proba=None, y_test_original=None):
        ...
        confusion_file = os.path.join(confusion_dir, 'matriz_confusion_modelo.png')
        plt.savefig(confusion_file, dpi=300, bbox_inches='tight')
        plt.show()

        print(f"Matriz de confusión guardada en: {confusion_file}")

        # otros_df vacío para compatibilidad (puedes llenarlo si lo necesitas)
...
def main():
    ...
    tipo_scaler_cat = "label_encoder'"
    modelo_svm.select_scaler("standard")
    modelo_svm.feature_selector = "kbest"
    dataset = "cic"
    if dataset == "cic":
        modelo_svm.select_scaler("standard")
        X_train_cic = modelo_svm.cargar_datos_CIC()
        y_train = X_train_cic['Label']
        X_train_cic = X_train_cic.drop('Label', axis=1)    
        X_train, X_test, y_train, y_test = train_test_split(
            X_train_cic, y_train, test_size=0.3, stratify=y_train
        )
        X_train = modelo_svm.preprocesar_datos_cic(X_train, train=False, limpiar_datos=True)
        X_test = modelo_svm.preprocesar_datos_cic(X_test, train=False, limpiar_datos=True)    
        best_features = modelo_svm.seleccionar_best_features(X_train, y_train)
        X_train = X_train[best_features]
        X_test = X_test[best_features]
        X_train = modelo_svm.preprocesar_datos_cic(X_train)
        X_test = modelo_svm.preprocesar_datos_cic(X_test, train=False)
        X_train, y_train  = modelo_svm.balancear_datos(X_train, y_train)
        modelo_svm.entrenar_modelo(X_train, X_test, y_train, y_test)
        return

    X_train = modelo_svm.cargar_datos()
    X_train.fillna('normal', inplace=True)
    ...
    X_train = modelo_svm.preprocesar_datos_unsw(X_train, train=True)
    X_test = modelo_svm.preprocesar_datos_unsw(X_test, train=False)
    ...
    X_train, y_train = modelo_svm.balancear_datos(X_train, y_train)
    modelo = modelo_svm.entrenar_modelo(X_train, X_test, y_train, y_test)
